defmodule PentoWeb.ProductLive.FormComponent do
  ...
  defp save_product(socket, :edit, params) do
    ...
    case Catalog.update_product(socket.assigns.product, product_params) do
      {:ok, _product} ->
        {:noreply,
          socket
          |> put_flash(:info, "Product update successfully")
          |> push_patch(to: socket.assigns.patch)}

      {:error, %Ecto.Changeset{} = changeset} ->
        {:noreplay, assign(socket, :changeset, changeset)}
    end
  end
  ...
  defp upload_static_file(%{path: path}, _entry) do
    # Plug in your image file persistence impl here!
